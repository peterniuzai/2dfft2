...
#Finally,return the data sets 
	...
	#   print d_sets[0].max(),np.nanmax(d_sets[0]),'4444'    
           freq = comm.bcast(freq,root=0)
...
#           time = comm.scatter(t_sets , root = 0)
	   time =  comm.scatter(t_sets , root = 0)
	   #print data.max(),data.min(),'55555',np.nanmax(data),np.nanmin(data)
           comm.barrier()
           d_sets_loc = []
           t_sets_loc = []
           for i in range(p_n):
...
#     plot_dir  = '/home/nch/search/wiggle_graph/' + str(cut_id)+'/'
     plot_dir  = '../graph/tem11/'
...
##############################################################################################
     ...
     if comm_rank == 0: 
		...
		d_sets , t_sets, freq, p_n ,datatype = read_data(f_dir, f_name , t_len, comm_size)
     else:
	  	datatype = p_n = freq = d_sets = t_sets = None
