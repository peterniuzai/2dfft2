...
from readfile1 import read_data
...
#Finally,return the data sets 
     if p_n < 1 :
	   ...
	   d_sets_loc = comm.bcast( d_sets,root =0)
           ...
           freq = comm.bcast(freq,root=0)
           data = np.empty((len(freq),t_len*p_n),dtype = datatype)
...
#     f_name    = 'B0329+54_filtered.npy'
     f_name    = 'Crab_52277_022_long.npy'
...
####################################################################################################
     ...
     t_len     = 128			#time length for each smallest unit to process.
     rad_grid  = 512			#radius grid size for interpolate in polar-coordin transform.
     ang_grid  = 850			#angle grid size for interpolate in polar-coordin transform.
